...
#------------------------------
# v.10.1
# This version just builds on 10.0. 10.0 cannot handle anything other than a distance
#	for each source; it is thus incomplete and cannot actually handle the anisotropic
#	.form files--it can only handle isotropic .form files which have no specified
#	source loading efficiency. 10.1 can handle all of them.
#
# It is currently being edited so that a random file can be formed, then run through
#	with 2D_run-10.0.py
#
#==============================================================================
import os
...
file_choice = raw_input("(I)nput info, or (G)ive a file name?  ").upper()
if file_choice=='I':
	hole = raw_input("Site? Careful with capitalization:  ")
	defined_file = hole+'.form'
elif file_choice=='G':
	defined_file = raw_input("Please enter name of file:  ")
...
for dataset in data_list:
	...
	# Getting .form file for specific dataset:
	file_ctr = 0
	directory = os.listdir(path)
	for thing in directory:
		if file_choice=='I':
			if (hole in thing) and (dataset in thing) and ('.form' in thing):
				wanted_file = thing
				file_ctr += 1
		elif file_choice=='G':
				if defined_file.upper()==thing.upper():
					wanted_file = thing
					file_ctr += 1
	if file_ctr != 1:
		if file_ctr > 1:
			print str(file_ctr)+" .form files match. Please check filenames."
			exit()
		elif file_ctr==0:
			print "No .form file found."
			exit()
		...
		if line[0] != '#':
			...
			if line[:6].lower()=='source':
				...
				if len(linesplit) == 4:
					...
					distance = float(linesplit[3])
				if source not in sources:
					sources.append(source)
					source_dist[source] = []
					source_lines[source] = []
				if line not in source_lines[source]:
					source_lines[source].append(line)
				if distance not in source_dist[source]:
					source_dist[source].append(distance)
			else:
				lines.append(line)
	file_form.close()

	for source in sources:
		source_dist[source].sort()
	print sources
	print source_dist
	print lines
	...
	if no_sources==1:
		...
		for source in sources:
			if file_choice=='I':
				...
				namesplit = wanted_file.split('.')
				pretitle = namesplit[0]
				title = pretitle+'--'+str(no_sources)+alphabet[ctr]+'.input'
...
### Creating .input files with more than one source for file.
	if choice2=='A':
		...
		if choice3=='1':
			for source in sources:
				ordered_distances.append(source_dist[source][0])
			...
			while ii < len(sources):
				...
				for source in sources:
					if source_dist[source][0]==distance_to_compare:
			...
			for source in sources:
				ordered_distances.append(source_dist[source][-1])
			ordered_distances.sort()
			if len(ordered_distances) != len(sources):
				print "ordered_distances != sources!"
				exit()
			...
			while ii < len(sources):
				...
				for source in sources:
					if source_dist[source][-1]==distance_to_compare:
						if source not in ordered_sources:
							ordered_sources.append(source)
				ii += 1
			...
			source_avgs = {}
			for source in sources:
				if len(source_dist[source])==1:
					source_avgs[source] = source_dist[source][0]
				else:
					min = source_dist[source][0]
					max = source_dist[source][-1]
					avg = (min+max)/2.
...
#		print ordered_distances
		...
		while no_sources <= len(sources):
			...
			if file_choice=='I':
				...
				namesplit = wanted_file.split('.')
				pretitle = namesplit[0]
				title = pretitle+'--'+str(no_sources)+'.input'
