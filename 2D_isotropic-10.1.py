...
#	simplicity.
					...
					errror_pphi = np.sqrt((p_error**2 + phi_error**2)/2.)
					...
					if errror < errorcap:
						...
						tmp_p_errs.append(p_error)
						tmp_phi_errs.append(phi_error)
			mins = minfinder(tmp_gamlist,tmp_etalist,tmp_errlist)
			mins2 = minfinder(tmp_p_errs,tmp_phi_errs, tmp_errlist)
			ii = 0
			while ii < len(mins[0]):
				pt_gam = str(mins[0][ii])
				pt_eta = str(mins[1][ii])
				pt_err = str(mins[2][ii])
				pt_perr = str(mins2[0][ii])
				pt_phierr = str(mins2[1][ii])
				check = str(mins2[2][ii])
				if check != pt_err:
					print "Error lists and value lists don't match."
					exit()
				point = pt_gam+','+pt_eta+','+pt_perr+','+pt_phierr+','+pt_err+' \n'
			...
			perr_dict[tide_final] = mins2[0]
			phierr_dict[tide_final] = mins2[1]
			
			
			results = minfinder(mins[0],mins[1],mins[2])
			err_results = minfinder(mins2[0],mins2[1],mins2[2])
			iii = 0
			while iii < len(results[0]):
				res_gam = str(results[0][iii])
				res_eta = str(results[1][iii])
				res_err = str(results[2][iii])
				res_perr = str(err_results[0][iii])
				res_phierr = str(err_results[1][iii])
				res_point = res_gam+','+res_eta+','+res_perr+','+res_phierr+','+res_err+' \n'
