...
#
# Uses new way to calculate error values--tries them across a numpy 2D array
#	using function from extrema.py. Errorcap not needed (HASN"T BEEN REMOVED YET)
#
#==============================================================================
import numpy as np
...
from extrema import 2D_min_brute as minfinder
...
for tide in tide_list:
	...
	for combo in iteration_list:
		error_arr = np.zeros((len(gamlist_init),len(etalist_init))
...
#		print iteration_trash
		...
		gam_index = 0 #row number

		for gam in gamlist_init:
			eta_index = 0 #column number
